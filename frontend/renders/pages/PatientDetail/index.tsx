  ...
  return (
    <DashboardLayout>
      <div className="patient-detail-page">
        ...
        <section className="questionnaire-answers-section">
          <h2>Respostas do Questionário Inicial</h2>
          {patient?.hasCompletedQuestionnaire ? (
            <div className="answers-list">
              {questions.map(q => (
                <div key={q.id} className="answer-item">
                  <p className="question-text">{q.text}</p>
                  <p className="answer-text">{patient.questionnaireAnswers?.[q.id] || 'Não respondido'}</p>
                </div>
              ))}
            </div>
          ) : (
            <p>O paciente ainda não preencheu o questionário inicial.</p>
          )}
        </section>
